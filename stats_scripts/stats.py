...
def get_gamma(G):
    ...
    gamma = vals[0]; R2 = vals[2]**2; p = vals[3]
    return gamma,R2,p
...
#    pooled_var = (np.var(x)*(len(x)-1) + np.var(y)*(len(y)-1))/(len(x)+len(y)-2)
    pooled_var = np.std(x)*np.std(y) # think this is the unbiased version
...
def distance_cc_corr(G):
    ...
    rho = distance_clustering[0]
    p =  distance_clustering[1]
...
if __name__ == "__main__":
    ...
    centroids_mat = mds.fit_transform(distance_matrix)
    centroids = {labels[i]:centroids_mat[i] for i in range(len(labels))}
    G = nx.relabel_nodes(G,new_labels)
    G.centroids = centroids


    # In the connectome, clustering coefficient can be expressed as a power of degree:
    # C ~ k^gamma, where gamma = -0.44
    # Also give corresponding values for SGPA and SGPAPA networks
    ...
    # Power laws for SGPA and TAPA
    if run_graphs:
        cc_deg_stats = {'SGPA':np.zeros((n_runs,3)),'TAPA':np.zeros((n_runs,3))}
        deg_stats = {'SGPA':np.zeros((n_runs,2)),'TAPA':np.zeros((n_runs,2))}        
        graphs = {'SGPA':L,'TAPA':L}
        dist_cc_stats = {'SGPA':np.zeros((n_runs,2)),'TAPA':np.zeros((n_runs,2))}
        
    models = {'SGPA':source_growth,'TAPA':target_attraction}
    for g in graphs:
        if run_graphs:
            for k in range(n_runs):
                ...
                G_mod, A_mod, labels_mod = model(N=num_brain_nodes,
                                        N_edges=num_brain_edges_directed,
                                        L=L, gamma=1., brain_size=[7.,7.,7.])
    ...
    print 'Connectome: t(%i)=%.2f, p=%.2e, Cohen\'s d=%.2f'%(df,t[0],t[1],d)
