...
if os.path.isfile(SAVE_FILE_NAME):
    ...
    for g_ctr in range(N_GRAPH_SAMPLES):
        ...
        Gs['rand'], _, _ = random_directed_deg_seq(
            in_sequence=in_degree_brain, out_sequence=out_degree_brain, simplify=True
        )

        # create target attraction graph
        Gs['ta'], _, _ = ta(
            N=bc.num_brain_nodes, N_edges=bc.num_brain_edges_directed, L=L, brain_size=BRAIN_SIZE,
        )
        ...
        Gs['sg_topo'], _, _ = sg(
            N=bc.num_brain_nodes, N_edges=bc.num_brain_edges_directed, L=np.inf, brain_size=BRAIN_SIZE,
        )

        # create purely geometric graph
        Gs['geom'], _, _ = sg(
            N=bc.num_brain_nodes, N_edges=bc.num_brain_edges_directed, L=L, gamma=0, brain_size=BRAIN_SIZE,
        )
...
for a_ctr, ax in enumerate(axs):
    ...
    if a_ctr == 0:
        ...
        ax.scatter(deg, cc, s=SCATTER_SIZE, lw=0, alpha=ALPHA_DEG_VS_CC, c=COLORS['brain'], zorder=1000)
    ...
    if a_ctr == 0:
        ...
        for key, G in zip(PLOT_KEYS, Gs):
            ...
            ax.scatter(deg, cc, s=SCATTER_SIZE, lw=0, alpha=ALPHA_DEG_VS_CC, c=COLORS[key])
        ...
        for key in PLOT_KEYS:
            line = ax.plot(
                BINCS_NODAL_EFFICIENCY, counts_nodal_efficiency_mean[key], color=COLORS[key], lw=LW,
            )
            ax.fill_between(
...
lines = [line[0] for line in lines_temp] + [hist_connectome[-1][0]]
...
for ax, text in zip(axs, texts):
    ...
    ax.text(
        0.05, 0.95, text, fontsize=20, fontweight='bold',
